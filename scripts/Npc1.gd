...
@export var dialog_state: int = 0  # 0 - первая встреча, 1 - после получения задания, 2 - после выполнения
...
# === ФУНКЦИИ ДИАЛОГОВ (остаются почти без изменений) ===
...
# === ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ХАРАКТЕРА ===
# Если NPC должен двигаться или анимироваться, добавьте сюда
...
func set_dialog_state(new_state: int):
	"""Установить состояние диалога извне (например, из другого скрипта)"""
	dialog_state = new_state
	print(npc_name + ": состояние диалога изменено на " + str(new_state))
